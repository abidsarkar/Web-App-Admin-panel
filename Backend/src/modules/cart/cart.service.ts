...
export const getCartService = async (userId?: string, sessionId?: string) => {
  let cart;
...
export const addToCartService = async (
  ...
  const { productId, quantity, size, color, colorCode } = data;
  ...
  const product = await ProductModel.findOne({
    ...
    "productId productName productPrice productStock productSize productColor productColorCode productCoverImage isDisplayable isSaleable"
  );
  ...
  // Check if item already exists in cart - FIXED TYPING
  ...
  if (existingItemIndex > -1) {
    ...
    cart.items[existingItemIndex].totalPrice =
      (product.productPrice || 0) * newQuantity; // FIXED: productPrice might be undefined
  } else {
    // Add new item - FIXED TYPING with proper null checks
    const cartItem: ICartItem = {
      productId: product.productId!, // FIXED: Using non-null assertion since we found the product
      productObjectId: product._id,
      productName: product.productName!, // FIXED: Using non-null assertion
      pricePerUnit: product.productPrice || 0,
      quantity,
      totalPrice: (product.productPrice || 0) * quantity,
      ...
      size: size || product.productSize || undefined,
      color: color || product.productColor || undefined,
      colorCode: colorCode || product.productColorCode || undefined,
    };
...
const calculateCartTotalsService = async (cart: any) => {
  ...
  cart.cartTotal = cart.items.reduce((sum: number, item: ICartItem) => { // FIXED: Added ICartItem type
